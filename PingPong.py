...
gameIcon = pygame.image.load('PingPong.jpg')
...
otherfont = pygame.font.Font(None, 32)
font = pygame.font.SysFont("comicsansms", 35)
scorefont = pygame.font.SysFont("Helvetica", 20)
gamecolor = (0, 0, 0)
# red name box
redname = ''
redcolor_active = pygame.Color('red3')
redcolor_passive = pygame.Color("gray15")
redcolor = redcolor_passive
redinput = pygame.Rect(100, 400, 140, 32)
redactive = False


# blue name box
bluename = ''
bluecolor_active = pygame.Color('turquoise3')
bluecolor_passive = pygame.Color("gray15")
bluecolor = bluecolor_passive
blueinput = pygame.Rect(100, 600, 140, 32)
blueactive = False
# color mode box
whichmode = ''
whichmode_active = pygame.Color('seashell')
whichmode_passive = pygame.Color("gray15")
whichmodecolor = whichmode_passive
whichmodeinput = pygame.Rect(100, 500, 140, 32)
whichmodeactive = False
...
def addscoretext(message, xpos, ypos, colorx):
    scoretext = scorefont.render(message, True, colorx, (255, 255, 255))
    scoretextrect = scoretext.get_rect()
    scoretextrect.topleft = xpos, ypos
...
ballY = 200
speeds = [7, 8, 9]
Yspeeds = [-4, -3, -2, -1, 0, 1, 2, 3, 4]
negYspeeds = [-4, -3, -2, -1, 0]
posYspeeds = [0, 1, 2, 3, 4]
ballVel = random.choice(speeds)
ballYvel = random.choice(Yspeeds)
radius = 10
# --------------------
...
run = True
while run:
    colors = pygame.Color(next(THECOLORS))
    ...
    if loadingscreen:
        ...
        textcolor = colors
        addtext("Welcome to Ping-Pong", 70, 150, textcolor, "colonna", 50)
        addtext("Press ENTER to start!", 75, 200, textcolor, "colonna", 50)

        red_surface = otherfont.render(redname, True, (255, 255, 255))
        blue_surface = otherfont.render(bluename, True, (255, 255, 255))
        whichmode_surface = otherfont.render(whichmode, True, (255, 255, 255))

        pygame.draw.rect(window, redcolor, redinput, 2)
        pygame.draw.rect(window, bluecolor, blueinput, 2)
        pygame.draw.rect(window, whichmodecolor, whichmodeinput, 2)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
            if event.type == pygame.MOUSEBUTTONDOWN:
                if redinput.collidepoint(event.pos):
                    redactive = True
                elif blueinput.collidepoint(event.pos):
                    blueactive = True
                elif whichmodeinput.collidepoint(event.pos):
                    whichmodeactive = True
                else:
                    redactive = False
                    blueactive = False
                    whichmodeactive = False
            ...
            if event.type == pygame.KEYDOWN:
                if redactive:
                    if event.key == pygame.K_BACKSPACE:
                        ...
                        redname += event.unicode
                elif blueactive:
                    if event.key == pygame.K_BACKSPACE:
                        bluename = bluename[:-1]
                    else:
                        bluename += event.unicode
                elif whichmodeactive:
                    if event.key == pygame.K_BACKSPACE:
                        whichmode = whichmode[:-1]
                    else:
                        whichmode += event.unicode

        if redactive:
            blueactive = False
            whichmodeactive = False
            redcolor = redcolor_active
        else:
            redcolor = redcolor_passive
        if blueactive:
            redactive = False
            whichmodeactive = False
            bluecolor = bluecolor_active
        else:
            bluecolor = bluecolor_passive
        if whichmodeactive:
            redactive = False
            blueactive = False
            whichmodecolor = whichmode_active
        else:
            whichmodecolor = whichmode_passive

        addtext("Put your name here, Red Player!", 100, 350, pygame.Color("hotpink"))
        window.blit(red_surface, (redinput.x + 5, redinput.y + 5))
        redinput.w = max(100, red_surface.get_width() + 10)

        addtext("Put your name here, Blue Player!", 100, 550, pygame.Color("hotpink"))
        window.blit(blue_surface, (blueinput.x + 5, blueinput.y + 5))
        blueinput.w = max(100, blue_surface.get_width() + 10)

        addtext("Type in light or dark to choose your mode!", 100, 450, pygame.Color("hotpink"))
        window.blit(whichmode_surface, (whichmodeinput.x + 5, whichmodeinput.y + 5))
        whichmodeinput.w = max(100, whichmode_surface.get_width() + 10)

        loadingkey = pygame.key.get_pressed()
        if loadingkey[pygame.K_RETURN] and redname != '' and bluename != '' and \
                (whichmode == "light" or whichmode == "Light" or whichmode == "dark" or whichmode == "Dark"):
            gameOn = True
    # ---------------

    if whichmode == "light" or whichmode == "Light":
        gamecolor = (255, 255, 255)

    if whichmode == "dark" or whichmode == "Dark":
        gamecolor = (0, 0, 0)
    ...
    if gameOn:
        pygame.time.delay(50)
        loadingscreen = False
        window.fill(gamecolor)
        # Title
        addtext("Ping-Pong", 200, 0, (0, 255, 0), "comicsansms")
        addtext("Press M to return to menu", 200, 650, pygame.Color("hotpink"), "helvetica", 32)

        # score displays
        addscoretext(f"Red Score = {redpoint}", 400, 0, (0, 0, 0))
        addscoretext(f"Blue Score = {bluepoint}", 400, 20, (0, 0, 0))

        # panels one and 2
        panelOne = pygame.draw.rect(window, (255, 0, 0), (paneloneX, paneloneY, panelWidth, panelHeight))
        panelTwo = pygame.draw.rect(window, (0, 0, 255), (paneltwoX, paneltwoY, panelWidth, panelHeight))

        # red and blue username displays
        addtext(redname, paneloneX + 5, paneloneY + panelHeight + 5, pygame.Color("lightgreen"))
        pygame.draw.rect(window, colors, (paneloneX, paneloneY + panelHeight, redinput.w, redinput.h), 2)
        addtext(bluename, paneltwoX + 5, paneltwoY + panelHeight + 5, pygame.Color("lightgreen"))
        pygame.draw.rect(window, colors, (paneltwoX, paneltwoY + panelHeight, blueinput.w, blueinput.h), 2)

        # ball and border
        ball = pygame.draw.circle(window, colors, (ballX, ballY), radius)
        box = pygame.draw.lines(window, colors, True, [(50, 650), (50, 50), (550, 50), (550, 650)])
        ...
        # controls and borders for the panels ------------------------------
        if keys[pygame.K_w] and paneloneY > 50:
            paneloneY -= vel

        if keys[pygame.K_s] and paneloneY < 650 - panelHeight:
            paneloneY += vel

        if keys[pygame.K_UP] and paneltwoY > 50:
            paneltwoY -= vel

        if keys[pygame.K_DOWN] and paneltwoY < 650 - panelHeight:
            paneltwoY += vel
        # -------------------------------------------------------

        # ball-collisions and borders
        if ballMoveRight and ballX <= 550 - (2 * radius) and 65 <= ballY <= 650 - radius:
            ballX += ballVel
            ballY += ballYvel

        if ballMoveLeft and ballX >= 50 + (2 * radius) and 65 <= ballY <= 650 - radius:
            ballX -= ballVel
            ballY += ballYvel

        if panelOne.collidepoint(ballX, ballY):
            ballMoveRight = True
            ballMoveLeft = False
            if ballY < 350:
                ballYvel = random.choice(posYspeeds)
            if ballY >= 350:
                ballYvel = random.choice(negYspeeds)

        if panelTwo.collidepoint(ballX, ballY):
            ballMoveRight = False
            ballMoveLeft = True
            if ballY < 350:
                ballYvel = random.choice(posYspeeds)
        ...
        if ball.collidepoint(540, ballY):
            question1 = scorefont.render("Press R to Continue", True, (0, 0, 0), (255, 255, 255))
            question = pygame.transform.scale(question1, (500, 75))
            questionRect = question1.get_rect()
            questionRect.topleft = 0, 350
            window.blit(question, questionRect)
            pygame.display.update()
            redpoint += 1
            pygame.event.wait()

        if ball.collidepoint(60, ballY):
            question1 = scorefont.render("Press R to Continue", False, (0, 0, 0), (255, 255, 255))
            question = pygame.transform.scale(question1, (500, 75))
        ...
        if ball.collidepoint(ballX, 650 - radius) or ball.collidepoint(ballX, 60):
            question1 = scorefont.render("Press R to Reset", False, (0, 0, 0), (255, 255, 255))
            question = pygame.transform.scale(question1, (500, 75))
            questionRect = question.get_rect()
            questionRect.topleft = 0, 350
            window.blit(question, questionRect)
    ...
    if bluepoint == goalreq:
        gameOn = False
        loadingscreen = False
        addtext("Congrats Blue!!! You win with", 80, 300, (0, 0, 255), "comicsansms")
        addtext(f"{bluepoint} points!", 240, 350, (0, 0, 255), "comicsansms")
    if redpoint == goalreq:
        gameOn = False
        loadingscreen = False
        addtext("Congrats Red!!! You win with", 80, 300, (255, 0, 0), "comicsansms")
        addtext(f"{redpoint} points!", 240, 350, (255, 0, 0), "comicsansms")
    # ------------------------
